...
This report consolidates the main technical findings identified in the repository, with a focus on violations of SOLID principles, code smells, and scalability/maintainability risks. Each finding includes its description, the violated principle or practice, and the impact on scalability.
...
- Impact on scalability:
  - Fragile API contracts: database schema changes break clients.
...
- Finding:
  - `TransactionController` orchestrates business logic and messaging (RabbitMQ), invoking a producer directly.
  - Evidence:
...
- Impact on scalability:
  ...
  - Greater coupling, less flexibility to change the broker or event publishing strategy.  
...
- Finding:
  - Names of `exchange`, `queues`, and `routing keys` are defined as string literals in code and repeated across microservices.
  - Evidence:
...
- Violated principle:
  - DRY and Single Source of Truth for frontend models.
- Impact on scalability:
  - Type inconsistencies as modules/teams grow.  
  - Increased change and coordination cost.  
  - Hinders reuse and typing quality.
